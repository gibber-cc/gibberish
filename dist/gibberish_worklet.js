  ...
  handleMessage( event ) {
    if( event.data.address === 'add' ) {
      ...
      if( event.data.name === 'Gibberish.ctx.sampleRate' ) {
        ...
        console.log( 'sample rate = ', processor.sampleRate ) 
      }else{
        const dict = event.data
        const obj = this.ugens.get( dict.object )
        obj[ dict.name ] = dict.value
